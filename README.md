...
2-3
- [ ] 구조체(struct)와 클래스(class) 차이점을 학습, 속성이 바뀔떄 어떠한 변화가 있는가 (학습-1)
- [ ] 참조 접근자를 활용해서 정보를 감추고 메소드 인터페이스를 통해 접근하는 방식을 학습 (학습-2)
- [ ] 개발 환경에서 제공하는 메모리를 분석하는 디버깅 도구는 무엇이 있는지 학습 (학습-3)
- [ ] 다양한 shuffle 알고리즘에 대해 찾아본다 (학습-4)
