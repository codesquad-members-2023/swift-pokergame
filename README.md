...
- [x] 참가자
- [x] 게임 규칙 (5, 7)
- [x] PokerGame에 딜러 참가자 포함
- [x] 2~5 사이의 영문의 이름만 추가가 가능한지
- [x] XCTest
...
Dealer-->Deck;
```
현재는 게임을 시작하면 카드를 게임 방식에 따라 5, 7개를 배분합니다.
게임을 종료하면 Player의 cards를 빈 배열로 만들고 deck을 reset합니다.

<!-- ### 게임의 시작
게임을 시작하는 방법은 두 가지이며, 게임이 시작되면 공통적으로 게임 방식에 따라 초기 카드가 배분된다.  
1. 플레이어 수를 정하고 새로운 게임을 시작하기   
startGame(with playerCount: Int)을 통해 플레이어 수를 정하고 게임을 시작한다. 
2. 플레이어를 유지하고 새로운 게임을 시작하기
startGame()을 통해 플레이어를 유지하고 게임을 시작한다. -->